...
class Eventplaneinfov1 : public Eventplaneinfo
{
 public:
  ...
  void set_flowvector(std::vector<std::pair<double,double>> Qvec) override { mQvec = Qvec;} 
  std::pair<double, double> get_flowvector(int order) const override {return std::make_pair(mQvec[order - 1].first, mQvec[order - 1].second);}
