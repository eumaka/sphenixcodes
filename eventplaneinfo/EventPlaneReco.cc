  ...
  : SubsysReco(name)
{
  ...
  mean_cos_south.resize(m_MaxOrder);
  mean_sin_south.resize(m_MaxOrder);
  mean_cos_north.resize(m_MaxOrder);
  mean_sin_north.resize(m_MaxOrder);
...
int EventPlaneReco::Init(PHCompositeNode *topNode)
{
    //get correction histograms
     TFile *mCorrectionInputFile = new TFile(OutFileName,"READ");
     if (mCorrectionInputFile->IsZombie())
     {
      std::cout << "No recentering hists found, out put will be raw Q vectors" << std::endl;
     ...
     else
     {
      for (unsigned int order = 1; order < m_MaxOrder + 1; order++)
      {
        tprof_mean_cos_south_mbd_input[order - 1] = (TProfile*)mCorrectionInputFile->Get(Form("tprof_mean_cos_south_mbd_order_%d",order));
        tprof_mean_sin_south_mbd_input[order - 1] = (TProfile*)mCorrectionInputFile->Get(Form("tprof_mean_sin_south_mbd_order_%d",order));
        tprof_mean_cos_north_mbd_input[order - 1] = (TProfile*)mCorrectionInputFile->Get(Form("tprof_mean_cos_north_mbd_order_%d",order));
        tprof_mean_sin_north_mbd_input[order - 1] = (TProfile*)mCorrectionInputFile->Get(Form("tprof_mean_sin_north_mbd_order_%d",order));
      }
     }
    ...
    for (unsigned int order = 1; order < m_MaxOrder + 1; order++)
    {
        tprof_mean_cos_south_mbd[order - 1] =  new TProfile(Form("tprof_mean_cos_south_mbd_order_%d", order), "", 100, 0, 3e3, -1e10, 1e10);
        tprof_mean_sin_south_mbd[order - 1] =  new TProfile(Form("tprof_mean_sin_south_mbd_order_%d", order), "", 100, 0, 3e3, -1e10, 1e10);
        tprof_mean_cos_north_mbd[order - 1] =  new TProfile(Form("tprof_mean_cos_north_mbd_order_%d", order), "", 100, 0, 3e3, -1e10, 1e10);
        tprof_mean_sin_north_mbd[order - 1] =  new TProfile(Form("tprof_mean_sin_north_mbd_order_%d", order), "", 100, 0, 3e3, -1e10, 1e10);
    }
...
int EventPlaneReco::process_event(PHCompositeNode *topNode)
{
  ...
  if (_mbdEpReco)
  {
    ...
    for (unsigned int order = 1; order < m_MaxOrder + 1; order++)
    {
        mean_cos_south[order - 1] = south_q[order - 1][0]/mbd_e_south;
        mean_sin_south[order - 1] = south_q[order - 1][1]/mbd_e_south;
        mean_cos_north[order - 1] = north_q[order - 1][0]/mbd_e_north;
        mean_sin_north[order - 1] = north_q[order - 1][1]/mbd_e_north;
                
        tprof_mean_cos_south_mbd[order - 1]->Fill(mbd_e_south,mean_cos_south[order - 1]);
        tprof_mean_sin_south_mbd[order - 1]->Fill(mbd_e_south,mean_sin_south[order - 1]);
        tprof_mean_cos_north_mbd[order - 1]->Fill(mbd_e_north,mean_cos_north[order - 1]);
        tprof_mean_sin_north_mbd[order - 1]->Fill(mbd_e_north,mean_sin_north[order - 1]);
